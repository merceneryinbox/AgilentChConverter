...
public class AgilentChFileParserNIO_2 {
	...
	// time scale start value set (represented as runtime computation so it might have been changed in using with another chromatography file)
	static int totalTimeValueInCurrentStep = getInitialTimePoint();
	// time scale step set (represented as runtime computation so it might have been changed in using with another chromatography file)
	static int timeDeltaIncrease = getTimeStep();

	// step of intensity abs set (represented as runtime computation so it might have been changed in using with another chromatography file)
	static int totalIntensityValueInCurrentStep = getInitialIntensityValue();
	// intensity steps set (represented as runtime computation so it might have been changed in using with another chromatography file)
	static int offsetDelta = getOffsetDelta();

	// start of valuable part of table hex = 1802, dec = 6146 (represented as runtime computation so it might have been changed in using with another chromatography file)
	static int inputFileByteReadPointOffset = getInitialInputFileStartByteReadPointOffset();
	static int loopCounter;

	// create new line for output (represented as runtime computation so it might have been changed in using with another chromatography file)
	...
	public static void main(String[] args) throws IOException {
		...
		byte[] fullChFileAsOneByteArray = Files.readAllBytes(Paths.get(args.length != 2 ? defaultChFilePath : args[1]));
		...
		try {
			...
			while (inputFileByteReadPointOffset < fullInputFileArrayLength) {
				...
				// read two bytes = absolute intensity value into two-bytes buffer from current offset of full input file buffer for further new line forming
				...
				// get converted from 2 bytes  big-endian DWORD hex string to -> unsigned int16 to -> signed int 16 = absolute intensity addition to total intensity
	...
	private static Path getOutPath(String[] args) throws IOException {
		Path filePath = Paths.get(args.length != getOffsetDelta() ? defaultOutputFile : args[1]);
		final Path pathParentFolder = filePath.getParent();

		if (pathParentFolder != null) {
			Files.createDirectories(pathParentFolder);
		}
		return filePath;
	}
